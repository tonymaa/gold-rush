...
//        logger.info(goldPriceAlerts.toString());
        for (GoldPriceAlert goldPriceAlert : goldPriceAlerts) {
            if (goldPrice.getBid() < goldPriceAlert.getLeftPoint()){
                ...
                String title = String.format("【黄金降价】清仓大甩卖，黄金降价啦，原本要580一克，现在仅需%s!!!", goldPrice.getBid());
                String content = String.format("来自%s的策略【%s】", user.getName(), goldPriceAlert.getDescription());
